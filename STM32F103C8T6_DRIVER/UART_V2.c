...
unsigned char Uart_Transmit ( unsigned char* u8_TxBuffer, unsigned int u4_Length )
{
	...
	{
		if( LengthBufferSend<u4_Length )
		{
			Software_Buffer_Send[LengthBufferSend] = u8_TxBuffer[LengthBufferSend];
			LengthBufferSend++;
		}
		else Transmit_State=BUSY;
	}
		...
		if(Index_Read == Index_Write) 
    {
      ...
      Receive_State = EMPTY;
		...
		else 
		{
			if(Index_Read != Index_Write)
			{
				u8_RxBuffer[LengthBufferRead] = Software_Buffer_Read[Index_Read];
        LengthBufferRead++;
